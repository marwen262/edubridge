...
function formatRel(d?: string): string {
  ...
  if (m < 1) return i18n.t('institution.notifications.timeAgo.justNow');
  if (m < 60) return i18n.t('institution.notifications.timeAgo.minutes', { count: m });
  const h = Math.floor(m / 60);
  if (h < 24) return i18n.t('institution.notifications.timeAgo.hours', { count: h });
  const j = Math.floor(h / 24);
  if (j < 7) return i18n.t('institution.notifications.timeAgo.days', { count: j });
...
export function InstitutionNotificationsSection() {
  ...
  const handleRead = async (id: string) => {
    ...
    catch { toast.error(t('institution.notifications.toasts.error')); }
  };
  const handleReadAll = async () => {
    setMarkingAll(true);
    try {
      await Promise.all(notifications.filter((n) => !n.est_lue).map((n) => notificationService.markAsRead(n.id)));
    ...
    <div>
      <div className="bg-white dark:bg-[#1D1D1F] border-b border-[var(--edu-border)] px-8 py-6">
        <div className="flex items-center justify-between flex-wrap gap-4">
          <div>
            <p className="text-xs font-semibold uppercase tracking-wider text-[var(--edu-text-tertiary)] mb-1">{t('institution.notifications.sectionLabel')}</p>
            <h1 className="text-3xl font-bold text-[var(--edu-text-primary)]">{t('institution.notifications.title')}</h1>
            <p className="text-sm text-[var(--edu-text-secondary)] mt-1">{t('institution.notifications.subtitle')}</p>
          </div>
          {unreadCount > 0 && (
            <Button onClick={handleReadAll} disabled={markingAll} className="rounded-full text-white" style={{ backgroundColor: 'var(--edu-indigo)' }}>
              <CheckCircle className="w-4 h-4 mr-2" /> {t('institution.notifications.markAllRead')}
            </Button>
          )}
        </div>
      </div>
      <div className="p-8 space-y-6 max-w-[1600px]">
        <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.35 }}
          ...
          <div className="relative flex-1">
            ...
            <input type="text" placeholder={t('institution.notifications.searchPlaceholder')} value={search} onChange={(e) => setSearch(e.target.value)}
          ...
          <div className="flex gap-2">
            {(['toutes', 'non_lues', 'lues'] as const).map((f) => (
              <button key={f} onClick={() => setFilter(f)}
                ...
                {f === 'toutes' ? t('institution.notifications.filters.all') : f === 'non_lues' ? t('institution.notifications.filters.unread') : t('institution.notifications.filters.read')}
              </button>
            ))}
          </div>
        </motion.div>
          ...
          <div className="divide-y divide-[var(--edu-divider)]">
            {loading ? Array.from({ length: 4 }).map((_, i) => (
              ...
              <div className="px-6 py-16 text-center">
                ...
                <p className="text-sm font-medium text-[var(--edu-text-primary)]">{t('institution.notifications.empty')}</p>
              </div>
            ) : paginated.map((n: Notification) => {
              const cfg = TYPE_STYLE[n.type] ?? TYPE_STYLE.systeme;
              const Icon = cfg.icon;
              const typeLabel = t(`institution.notifications.types.${n.type}`, { defaultValue: n.type });
              return (
                <div key={n.id} className={`px-6 py-5 flex items-start gap-4 hover:bg-[var(--edu-surface)] transition-colors ${!n.est_lue ? 'bg-[var(--edu-blue)]/[0.03]' : ''}`}>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap mb-0.5">
                      ...
                      <span className="text-[10px] uppercase tracking-wider font-semibold px-1.5 py-0.5 rounded" style={{ backgroundColor: cfg.bg, color: cfg.color }}>{typeLabel}</span>
                    ...
                    <div className="flex items-center gap-3 mt-2">
                      ...
                      {!n.est_lue && <button onClick={() => handleRead(n.id)} className="text-[11px] font-semibold text-[var(--edu-blue)] hover:underline">{t('institution.notifications.markAsRead')}</button>}
                    </div>
                  </div>
                </div>
              );
          ...
          {totalPages > 1 && (
            <div className="px-6 py-4 border-t border-[var(--edu-border)] flex items-center justify-between">
              <p className="text-xs text-[var(--edu-text-tertiary)]">
                {filtered.length} {filtered.length !== 1 ? t('institution.notifications.paginationPlural') : t('institution.notifications.pagination')} — {t('common.page')} {page}/{totalPages}
              </p>
