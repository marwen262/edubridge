...
const INST_STATUS_LABELS: Record<string, { label: string; cls: string }> = {
  approved:             { label: 'Approuvé',  cls: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300' },
  pending_admin_review: { label: 'En attente', cls: 'bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-300' },
  invited:              { label: 'Invité',     cls: 'bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-300' },
  rejected:             { label: 'Rejeté',     cls: 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300' },
  suspended:            { label: 'Suspendu',   cls: 'bg-gray-100 text-gray-600 dark:bg-gray-800/30 dark:text-gray-400' },
};
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center">
      ...
      <div className="relative w-full max-w-md bg-white dark:bg-[#1D1D1F] rounded-2xl shadow-2xl border border-[var(--edu-border)] overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        <div className="px-6 py-5 border-b border-[var(--edu-border)] flex items-center justify-between">
          <h2 className="text-lg font-bold text-[var(--edu-text-primary)]">Supprimer l'utilisateur</h2>
...
export function UsersSection() {
  ...
  // Statut affiché : pour les instituts, on montre le validation_status au lieu de est_actif
  const getStatut = (u: Utilisateur): { label: string; cls: string } => {
    if (u.role === 'institut' && u.institut?.validation_status) {
      const instStatus = INST_STATUS_LABELS[u.institut.validation_status];
      if (instStatus) return instStatus;
    }
    // Fallback : statut actif/désactivé
    return u.est_actif !== false
      ? { label: 'Actif', cls: 'bg-emerald-100 text-emerald-700 dark:bg-emerald-900/30 dark:text-emerald-300' }
      : { label: 'Désactivé', cls: 'bg-red-100 text-red-700 dark:bg-red-900/30 dark:text-red-300' };
  };
  ...
  return (
    <div>
      ...
      <div className="bg-white dark:bg-[#1D1D1F] border-b border-[var(--edu-border)] px-8 py-6">
        ...
        <p className="text-sm text-[var(--edu-text-secondary)] mt-1">Gestion complète des comptes de la plateforme</p>
      </div>
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-[var(--edu-divider)]">
                {loading ? (
                  ...
                  <tr><td colSpan={5} className="px-6 py-12 text-center"><Users className="w-8 h-8 mx-auto mb-2 text-[var(--edu-text-tertiary)]" /><p className="text-sm text-[var(--edu-text-secondary)]">Aucun utilisateur trouvé.</p></td></tr>
                ) : paginated.map((u) => {
                  const nom = getNom(u);
                  ...
                  return (
                    <tr key={u.id} className="hover:bg-[var(--edu-surface)] transition-colors">
                      ...
                      <td className="px-6 py-4">
                        <span className="text-sm text-[var(--edu-text-secondary)]">{u.cree_le ? new Date(u.cree_le).toLocaleDateString('fr-FR') : '—'}</span>
                      </td>
                      <td className="px-6 py-4">
                          ...
                          {actionMenu === u.id && (
                            <div className="absolute right-0 top-full mt-1 z-50 w-48 bg-white dark:bg-[#2D2D2F] rounded-xl shadow-xl border border-[var(--edu-border)] py-1 animate-in fade-in slide-in-from-top-1">
                              <button onClick={() => handleToggleActif(u)}
                                ...
                                {u.est_actif !== false ? (
                                  <><UserX className="w-4 h-4 text-[var(--edu-warning)]" /> Désactiver</>
                                ) : (
                                  <><UserCheck className="w-4 h-4 text-[var(--edu-success)]" /> Réactiver</>
                                )}
                              </button>
